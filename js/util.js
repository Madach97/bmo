...
/**
 * Provide static utility methods related to Bugzilla API access.
 * @see https://bugzilla.readthedocs.io/en/latest/api/
 * @todo Replace this with the complete implementation being developed in Bug 1380437.
 */
...
Bugzilla.String = class String {
   ...
   * @param {Boolean} [ignore_case=true] Whether to do a case insensitive match.
   * @param {Boolean} [find_all=true] Whether all the words should be contained in the string.
   * @returns {Boolean} Whether the words could be found in the string.
   */
  static find(string, words, ignore_case = true, find_all = true) {
    ...
    const finder = word => _string.includes(word);

    return find_all ? _words.every(finder) : _words.some(finder);
  }
