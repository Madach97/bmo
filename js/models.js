  ...
  static async fetch_products() {
    ...
    try {
      const { products } = await Bugzilla.API.get('product', {
        type: 'accessible',
        include_fields: 'name,description,components.name,components.description',
      });
  ...
  static async fetch_recent_bugs() {
    ...
    try {
      ...
      let { bugs } = await Bugzilla.API.get('bug', {
        id: last_visit.map(bug => bug.id).join(','),
        include_fields: 'id,alias,summary',
      });
      ...
      bugs = Bugzilla.Array.sort(bugs, 'last_visit', true);
